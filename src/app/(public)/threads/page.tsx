    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="bg-white rounded-xl shadow-sm border border-stone-200 p-5 hover:border-orange-300 transition-colors">
              <div className="flex justify-between items-start mb-2">
                <Link href={`/threads/thread-${i}`} className="text-lg font-bold text-stone-900 hover:text-orange-600">
                  What is the best time for VIP Darshan at Kashi Vishwanath?
                </Link>
                <span className="text-xs text-stone-400 whitespace-nowrap ml-4">2 hours ago</span>
              </div>
              <p className="text-sm text-stone-600 mb-4 line-clamp-2">
                I am planning a visit next month with elderly parents. Does anyone know if the early morning Mangala Aarti passes are available online, or do we need to queue up?
              </p>
              
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5">
                    <div className="w-6 h-6 rounded-full bg-orange-100 flex items-center justify-center text-orange-700 font-bold">R</div>
                    <span className="font-medium text-stone-700">Rahul M.</span>
                  </div>
                  <span className="text-stone-300">•</span>
                  <Link href="#" className="bg-stone-100 text-stone-600 px-2 py-0.5 rounded hover:bg-stone-200">
                    Temple Guidance
                  </Link>
                </div>
